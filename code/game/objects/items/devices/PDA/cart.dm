...
/obj/item/weapon/cartridge/proc/create_NanoUI_values(mob/user as mob)
	...
	if(mode==43 || mode==433)
		...
		for(var/obj/machinery/power/apc/A in L)
			apcData[++apcData.len] = list("Name" = rrhtml_encode(A.area.name), "Equipment" = Status[A.equipment+1], "Lights" = Status[A.lighting+1], "Environment" = Status[A.environ+1], "CellPct" = A.cell ? round(A.cell.percent(),1) : -1, "CellStatus" = A.cell ? chg[A.charging+1] : 0)
	...
	if(mode==47)
		...
		for(var/S in supply_controller.shoppinglist)
			...
			supplyOrderData[++supplyOrderData.len] = list("Number" = SO.ordernum, "Name" = rrhtml_encode(SO.object.name), "ApprovedBy" = SO.orderedby, "Comment" = rrhtml_encode(SO.comment))
		...
		for(var/S in supply_controller.requestlist)
			var/datum/supply_order/SO = S
			requestCount++
			requestData[++requestData.len] = list("Number" = SO.ordernum, "Name" = rrhtml_encode(SO.object.name), "OrderedBy" = SO.orderedby, "Comment" = rrhtml_encode(SO.comment))
		if(!requestData.len)
